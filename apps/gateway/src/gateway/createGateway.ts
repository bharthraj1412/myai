...
export async function createGateway(config: Config): Promise<Gateway> {
  ...
  const scheduler = new Scheduler(
    ...
    async (channelTarget, message) => {
      ...
      if (target) {
        ...
        const sessions = sessionManager.listSessions().filter(s => s.channelType === target.type || s.channelId === target.id);
        if (sessions.length === 0) {
          console.warn(`[Scheduler] No active sessions found for target ${target.id} to send notification.`);
        } else {
          for (const session of sessions) {
            try {
              await target.send(session.chatId, { text: message });
              console.log(`[Scheduler] Sent notification to chat ${session.chatId}`);
            } catch (err) {
              console.error(`[Scheduler] Failed to send notification to chat ${session.chatId}:`, err);
            }
          }
        }
      } else {
  ...
  // Mount PAI Memory API routes (Algorithm, TELOS, Ratings, Learnings)
  try {
    const { default: memoryRoutes } = await import("../routes/memory.js");
    app.use("/api/memory", memoryRoutes);
    gatewayLogs.info("Memory", "PAI Memory API routes mounted at /api/memory");
  } catch (err) {
    gatewayLogs.warn("Memory", `Memory routes not loaded: ${err}`);
  }

  // Initialize PAI Hook Manager
  let hookManager: any = null;
  try {
    const { HookManager } = await import("../hooks/hook-manager.js");
    hookManager = new HookManager(
      path.join(projectRoot, "config", "settings.json"),
      5000
    );
    hookManager.on('hook-error', (data: any) => {
      gatewayLogs.warn("HookManager", `Hook error: ${data.command} — ${data.error || data.stderr || ''}`);
    });
    hookManager.on('hook-output', (data: any) => {
      gatewayLogs.debug("HookManager", `Hook output [${data.eventName}]: ${data.output}`);
    });
    const totalHooks = hookManager.getTotalHookCount();
    gatewayLogs.info("HookManager", `PAI Hook Manager initialized (${totalHooks} hooks configured)`);
  } catch (err) {
    gatewayLogs.debug("HookManager", `Hook Manager not loaded: ${err}`);
  }
  ...
  // Get gateway status summary for control surfaces
  app.get(`${config.gateway.httpPath}/status`, async (_req, res) => {
    try {
      const model = getModelConfigSnapshot();
      const nodeStatus = nodeRegistry.getStatus();
      const schedulerStatus = scheduler.getStatus();
      const sessions = sessionManager.listSessions();
      ...
      let agentHealth: Record<string, unknown> = { status: "offline" };

      try {
        const agentResponse = await fetch(`http://localhost:18790/health`, { signal: controller.signal });
        clearTimeout(timeout);
        if (agentResponse.ok) {
          agentHealth = await agentResponse.json();
        } else {
          agentHealth = { status: "degraded", httpStatus: agentResponse.status };
        }
      } catch {
        clearTimeout(timeout);
      }
      ...
      await new Promise<void>((resolve, reject) => {
        const onError = (err: NodeJS.ErrnoException) => {
          ...
          if (err.code === "EADDRINUSE") {
            reject(
              new Error(
                `Gateway port ${config.gateway.port} is already in use. Stop the existing process or choose a different port.`
              )
            );
            return;
          }
