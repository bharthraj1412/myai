...
def install_skill(
    ...
    """Install a community skill from a repository or raw URL.
    
    Downloads the SKILL.md and associated files into the local workspace.
    
    Args:
        skill_slug: The unique slug or name for the skill
        repository_url: URL to the raw SKILL.md file or GitHub repository
        
    Returns:
        String status denoting success or error
    """
    try:
        # Determine raw URL (naive conversion for github if needed)
        url = repository_url
        ...
        target_dir = repo_root / ".ag3nt" / "skills" / skill_slug
        target_dir.mkdir(parents=True, exist_ok=True)
        
        target_file = target_dir / "SKILL.md"
        
        logger.info(f"Installing skill from {url} to {target_file}")
        ...
        # Note: A production implementation would also extract and download the actual 
        # script files listed in the entrypoints, but for community standard skills 
        # often the SKILL.md contains everything or just setup instructions.
        
        return f"Successfully installed skill '{skill_slug}' to {target_dir}. You can now run it using run_skill."
    except Exception as e:
        logger.error(f"Failed to install skill {skill_slug}: {e}")
...
def run_skill(
    ...
    try:
        ...
        elif base_script.suffix in (".js", ".mjs"):
            cmd = ["node", str(base_script)] + script_args
        elif base_script.suffix == ".ts":
            # Assume tsx or ts-node is available via npx
            cmd = ["npx", "tsx", str(base_script)] + script_args
        ...
        result = subprocess.run(
            ...
            capture_output=True,
            text=True,
            timeout=120,  # 120 second timeout for complex skills
        )
