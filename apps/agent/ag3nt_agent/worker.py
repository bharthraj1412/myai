...
def _healthy_agent_running(port: int) -> bool:
    ...
    try:
        with urlopen(f"http://127.0.0.1:{port}/health", timeout=2) as response:
            ...
            return payload.get("ok") is True and payload.get("name") == "ag3nt-agent"
    except Exception:
        return False
...
def main():
    ...
    port = 18790
    if _healthy_agent_running(port):
        print(
            f"[Agent] Port {port} is already in use by a healthy ag3nt-agent instance. Exiting without error."
        )
        return
