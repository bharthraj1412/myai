...
def browser_evaluate_js(script: str) -> str:
    """Evaluate arbitrary JavaScript within the browser page context.

    Args:
        script: The JavaScript code to execute. Can be a simple expression or block.

    Returns:
        The evaluated result, or a success message if execution didn't return a value.

    Example:
        browser_evaluate_js("document.title + ' - modified'")
        browser_evaluate_js("window.scrollTo(0, document.body.scrollHeight)")
    """
    async def _evaluate():
        bridge = await _try_bridge()
        if bridge:
            ...
            if resp.get("type") == "error":
                return f"Error evaluating JS: {resp.get('message', 'unknown')}"
            return f"JS Output: {resp.get('result', '(no result)')}"
...
@tool
def browser_scroll(direction: Literal["up", "down", "top", "bottom"] = "down", amount: Optional[int] = None) -> str:
    """Scroll the page in the specified direction.

    Args:
        direction: Direction to scroll ("up", "down", "top", "bottom").
        amount: Optional amount in pixels to scroll. If omitted, scrolls by 800px or to the extremeties.
    ...
    async def _scroll():
        scroll_script = ""
        scroll_amount = amount if amount is not None else 800
        ...
        if direction == "down":
            ...
            scroll_script = "window.scrollTo({top: 0, behavior: 'smooth'})"
        elif direction == "bottom":
            scroll_script = "window.scrollTo({top: document.body.scrollHeight, behavior: 'smooth'})"
        ...
        if bridge:
            ...
            if resp.get("type") == "error":
                return f"Error scrolling: {resp.get('message', 'unknown')}"
