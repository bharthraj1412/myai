...
def _build_agent() -> CompiledStateGraph:
    ...
    system_prompt = """You are AG3NT (AP3X), a personal AI assistant powered by PAI v2.0 architecture.

## Core Identity
You are a goal-oriented, self-improving AI with persistent memory. You learn from every interaction,
track work with PRD documents, and follow the 7-phase Algorithm for complex tasks.

## PAI Architecture
- **Algorithm Engine**: For complex tasks, follow Observe→Think→Plan→Build→Execute→Verify→Learn
- **Memory System**: Your learnings persist in MEMORY/ across sessions
- **TELOS**: The principal's goals live in USER/TELOS/ — align your work with their mission
- **Rating System**: When the user rates you 1-10, it's captured for trend analysis
- **Hooks**: Lifecycle events (session start/end, tool use) trigger automated side-effects
...
```"""
    ...
    # Initialize Algorithm middleware (PAI v3.7.0 integration)
    algorithm_middleware = None
    try:
        from ag3nt_agent.algorithm.middleware import AlgorithmMiddleware
        algorithm_middleware = AlgorithmMiddleware()
        logger.info("Algorithm Engine v3.7.0 initialized")
    except Exception as e:
        logger.debug(f"Algorithm middleware not available: {e}")
    ...
    if algorithm_middleware:
        middleware_list.append(algorithm_middleware)
...
def run_turn(
    ...
    # Route input through JARVIS Brain module for intent analysis
    try:
        from ag3nt_agent.brain import process_with_brain
        enhanced_text = process_with_brain(session_id, text)
    except Exception as e:
        logger.error(f"Failed to process via JARVIS brain: {e}")
        enhanced_text = text
    ...
    messages = [HumanMessage(content=enhanced_text)]
...
class AutonomousRuntime:
    ...
    async def _process_goal(self, goal, event) -> None:
        ...
        if decision.decision_type == DecisionType.ACT:
            ...
            from ag3nt_agent.agent_config import GATEWAY_URL
            try:
                action_data = goal.action.render(event)
                payload = {
                    "reason": decision.reason,
                    "goal_id": goal.id,
                    "action": getattr(action_data, 'command', str(action_data))
                }
                requests.post(
                    f"{GATEWAY_URL}/api/sessions/autonomous-{goal.id}/queue_approval",
                    json=payload,
                    timeout=5
                )
            except Exception as e:
                logger.error(f"Failed to queue approval for goal '{goal.name}': {e}")
