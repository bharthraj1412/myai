...
from deepagents_cli.widgets.tool_call_display import create_tool_display
...
class TextualUIAdapter:
    ...
    def __init__(
        ...
        # Dict of tool_id -> tool display widget (ToolCallMessage or ToolCallDisplayBase)
        self._current_tool_messages: dict[str, Any] = {}
        self._pending_text = ""
                ...
                # Handle MESSAGES stream - for content and tool calls
                    ...
                    for block in message.content_blocks:
                        ...
                        if block_type == "text":
                            text = block.get("text", "")
                            if text:
                                _log.debug(f"Text chunk received: {text[:100]!r}...")
                            ...
                            _log.debug(f"Tool call block received: type={block_type}, block={block!r}")
                            ...
                            if buffer_id is not None and buffer_id not in displayed_tool_ids:
                                ...
                                # Mount tool call message using new display system
                                _log.info(
                                    f"Creating tool display: name={buffer_name!r}, "
                                    f"args_keys={list(parsed_args.keys()) if parsed_args else []}"
                                )
                                tool_msg = create_tool_display(buffer_name, parsed_args)
                                _log.info(
                                    f"Mounting tool widget: type={type(tool_msg).__name__}, "
                                    f"id={id(tool_msg)}"
                                )
