...
from deepagents_cli.widgets.tool_call_display import ToolCallDisplayBase, create_tool_display
...
class DeepAgentsApp(App):
    ...
    TITLE = "DeepAgents"
    CSS_PATH = "app.tcss"
    ENABLE_COMMAND_PALETTE = True  # Enable command palette (Ctrl+P)
                ...
                if isinstance(msg, HumanMessage):
                    ...
                    for tc in tool_calls:
                        ...
                        if tc_id:
                            ...
                            # Mount tool call widget using new display system
                            tool_widget = create_tool_display(
                                tc.get("name", "unknown"),
                                tc.get("args", {}),
                            )
    ...
    def get_system_commands(self, screen: Screen) -> Iterable[SystemCommand]:
        ...
        # Include default system commands first
        yield from super().get_system_commands(screen)

        # Auto-approve toggle with current status
        status = "ON" if self._auto_approve else "OFF"
        yield SystemCommand(
            f"Toggle Auto-Approve Mode (currently {status})",
        ...
        yield SystemCommand(
            "Toggle Tool Output",
            "Expand or collapse the most recent tool output",
    ...
    def action_toggle_tool_output(self) -> None:
        ...
        # Try new ToolCallDisplayBase first, then fallback to old ToolCallMessage
        try:
            # Query for new display widgets
            tool_widgets = list(self.query(ToolCallDisplayBase))
            for tool_widget in reversed(tool_widgets):
                if tool_widget.has_output:
                    tool_widget.toggle_expanded()
                    return
            # Fallback to old ToolCallMessage for backward compatibility
